...
echo "htseq-count -m union -t exon -i transcript_id ${READS_1}_aligned_sorted.sam $HOME/Ecoli_RNAseq/reference_seqs/final_reference_seqs/REL606_nc.gtf  > ${READS_1}_raw_rna_count.txt" ##>> ${LOGFILE}
if [[ ! $TEST = "true" ]]; then
	htseq-count -m union -t exon -i transcript_id ${READS_1}_aligned_sorted.sam $HOME/Ecoli_RNAseq/reference_seqs/final_reference_seqs/REL606_nc.gtf  > ${READS_1}_raw_rna_count.txt ##2>> ${LOGFILE}
fi
#TEST="true"
...
if [[ ! $TEST = "true" ]]; then
	python $HOME/Ecoli_RNAseq/src/quality_control.py ${READS_1}.fastq ${READS_1}_trimmed.fastq ${READS_1}_R2_bowtie.out ${READS_1}_bowtie.out ${READS_2}_bowtie.out ${READS_1}_raw_rna_count.txt 
fi 
